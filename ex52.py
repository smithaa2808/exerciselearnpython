...
mission is the get the neutron destruct bomb from the Weapons Armory, put
it in the bridge, and blow the ship up after getting into an escape pod.

You're running down the central corrider to the Weapons Armory when a
...
You do dive roll into the Weapon Armory, crouch and scan the room for
...
"""
The container cicks open and the seal breaks, letting gas out. You
...
"""
You burst onto the Bridge with the neutron destruct bomb under your arm the Gothons put 
...
button and blast the lock so that Gothons can't get out. Now that the
bomb is placed you run to the escape pod to get off this tin can.

You rush through the ship desperrately trying to make it to the escape
pod before the whole ship explodes. It seems like hardly any Gothons
are on the ship, so your run is clear of interferences. Some of
...
"""
You jump into pod 2 and ht the eject button. The pod easily slides out
